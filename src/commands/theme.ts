...
export class Theme implements Command {
  ...
  async set(message: Message, argsArray: string[]): Promise<void> {
    ...
    const theme = argsArray[1].toLowerCase();

    if (theme !== 'light' && theme !== 'dark') {
      await message.channel.send('Theme name is incorrect.');
      return;
    }
  ...
  async show(message: Message, argsArray: string[]): Promise<void> {
    ...
    embed.addField('Theme: ', bgs.theme);
  ...
  help(): [string, string, string, string[]] {
    return [
      ...
      ['`@BGSBot theme set dark`', '`@BGSBot theme set light`', '`@BGSBot theme remove`', '`@BGSBot theme show`']
    ];
  }
}
